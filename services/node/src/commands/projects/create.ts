...
export class Create extends CommandBase implements ICommand {
    ...
    public async relax (initiative: IInitiative): Promise<string> {
        ...
        return PROJECT_MODEL.find({ name: initiative.data.name } as ProjectDocument).then(async (existent) => {
            if (existent.length > 0) {
                ...
                const project = PROJECT_MODEL.build(Object.assign(initiative.data, {
                    queues: [queue],
