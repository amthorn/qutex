...
class AuthRegisterApi(Resource):
    def post(self) -> dict[str, Union[list[dict], dict[str, str]]]:
        ...
        # And now we will set the password which can be used for further authentication in the future.
        ...
        if 'email' not in temp_token:
            # Should never occur unless something weird is going on where they are using a permanant JWT
        ...
        if len(possible_users) > 1:
            raise Conflict("More than one webex account found for that email address. Cannot resolve conflict.")
        ...
        # The basic PersonDocument.objects.get(id=user_data.id) does not work. Perhaps a bug in mongo engine
