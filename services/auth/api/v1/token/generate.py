...
class TokenGenerateApi(Resource):
    ...
    def post(self) -> dict[str, dict[str, bool]]:
        ...
        # That way, althought the token expires in 10 minutes,
        # the deletion of the message helps to regulate it's only
        ...
        # TODO: better way to manage all these hardcoded routes?
        invalidate_token_route = '/api/v1/auth/token/invalidate'
        bot._session.put(f'/messages/{message.id}', json={
            'markdown': 'Your temporary token is shown below. '
                        f'This token will expire in {self.expiration} minutes:'
                        # TODO: should delete message and invalidate (blacklist) token
                        f'\n\n{token}\n\nWas this not generated by you? '
                        f'[Click Here]({app.config["FQDN"]}{invalidate_token_route}?token={token})',
